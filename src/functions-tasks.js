...
function getArgumentsCount(funcs) {
  const result = [];
  funcs.forEach((_, index) => result.push(index));
  return result;
}
...
function getIdGeneratorFunction(startFrom) {
  let dontBotherMeLinter = startFrom;
  return () => {
    const result = dontBotherMeLinter;
    dontBotherMeLinter += 1;
    return result;
  };
}
